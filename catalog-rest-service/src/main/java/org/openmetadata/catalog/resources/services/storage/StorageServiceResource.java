...
@Collection(name = "storageServices", repositoryClass = "org.openmetadata.catalog.jdbi3.StorageServiceRepository")
public class StorageServiceResource {
    ...
    private static StorageService addHref(UriInfo uriInfo, StorageService storageService) {
        storageService.setHref(RestUtil.getHref(uriInfo, "v1/services/storageServices/", storageService.getId()));
        return storageService;
    }

    @Inject
    public StorageServiceResource(StorageServiceRepository dao, CatalogAuthorizer authorizer) {
    ...
    public StorageServiceList list(@Context UriInfo uriInfo, @QueryParam("name") String name) throws IOException {
        return new StorageServiceList(addHref(uriInfo, dao.list(name)));
    }
    ...
    public Response create(@Context UriInfo uriInfo,
                           @Context SecurityContext securityContext,
                           @Valid CreateStorageService create) throws JsonProcessingException {
        SecurityUtil.checkAdminOrBotRole(authorizer, securityContext);
        StorageService storageService = new StorageService().withId(UUID.randomUUID())
                .withName(create.getName()).withDescription(create.getDescription())
                .withServiceType(create.getServiceType());

        addHref(uriInfo, dao.create(storageService));
        return Response.created(storageService.getHref()).entity(storageService).build();
    }
    ...
    public Response update(@Context UriInfo uriInfo,
        ...
        SecurityUtil.checkAdminOrBotRole(authorizer, securityContext);
        StorageService storageService = addHref(uriInfo,
                dao.update(id, update.getDescription()));
        return Response.ok(storageService).build();
    }
