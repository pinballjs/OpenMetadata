...
public class TableResource {
  ...
  public Response createIfNotExists(@Context UriInfo uriInfo, @Context SecurityContext securityContext,
    ...
    checkCanCreateLocation(teamName, "s3." + create.getLocation());
    ...
    CreateLocation createLocation = new CreateLocation().withService(storageServiceRef)
            .withLocationType(LocationType.Table).withName(create.getLocation()).withOwner(owner);
    ...
    DatabaseService databaseService = OpenMetadataService.createDatabaseService(createDatabaseService,
            adminAuthHeaders());
  ...
  public static void checkCanCreateLocation(String teamName, String fqn) throws HttpResponseException {
    LocationList locations = OpenMetadataService.listPrefixes("owner", fqn, null, null, null, adminAuthHeaders());
    LOG.info("FQN1: " + fqn);
    List<String> owners = locations.getData().stream().map(l -> {
      // check location is upstream folder otherwise null.
      LOG.info("FQN2: " + l.getFullyQualifiedName());
      LOG.info("OWNER: " + l.getOwner().getName());
      if (l.getOwner() != null) {
    ...
    if (!owners.contains(teamName) && !owners.isEmpty()) {
      throw new AuthorizationException("Team: " + teamName + " is not the owner of " + fqn);
    }
  }
}